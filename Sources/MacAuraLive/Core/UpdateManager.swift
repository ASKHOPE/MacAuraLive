...
public class UpdateManager: ObservableObject {
    ...
    public let currentVersion = "1.8.0"
        ...
        URLSession.shared.dataTask(with: request) { [weak self] data, response, error in
            DispatchQueue.main.async {
                ...
                guard let data = data,
                    ...
                    self?.status = .upToDate(version: self?.currentVersion ?? "1.8.0")
                    return
                }
                ...
                if self?.isVersion(latestVersion, newerThan: self?.currentVersion ?? "1.8.0") == true {
